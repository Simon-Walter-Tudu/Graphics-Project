...
const double Cpi = 3.14159265389;
const int Cnt = 40;
const int Cntheta = 20;
...
public:
    ...
    unsigned int hollowBezier(GLfloat ctrlpoints[], int L, Shader& lightingShader)
    {
        ...
        const float dtheta = 2 * Cpi / Cntheta;        //angular step size

        float t = 0;
        float dt = 1.0 / Cnt;
        float xy[2];

        for (i = 0; i <= Cnt; ++i)              //step through y
        {
        ...
        for (int i = 0; i < Cnt; ++i)
        {
            k1 = i * (Cntheta + 1);     // beginning of current stack
            k2 = k1 + Cntheta + 1;      // beginning of next stack

            for (int j = 0; j < Cntheta; ++j, ++k1, ++k2)
            {
                // k1 => k2 => k1+1
